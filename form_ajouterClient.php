...
if(!empty($_POST)) {
    ...
    $req = $db->prepare('SELECT id FROM Personne_Client WHERE nom = :nom');
    ...
    if($res == false) {
        ...
        if($_POST['password'] == $_POST['password2']) {
            ...
            try {
                $req = $db->prepare('INSERT INTO Personne_Client(personne_id, nom, adresse, pays) VALUES(:personne_id, :nom, :adresse, :pays)');
            ...
            Flash::success("Le client a bien été ajouté");
...
<div class="jumbotron text-left">
    ...
    <form action="" method="post">
        <div class="form-group">
            <label for="input-name">Nom du client</label>
            <input type="text" name="name" class="form-control" id="input-name" placeholder="Nom">
        </div>
        
        <div class="form-group">
            <label for="input-address">Adresse du client</label>
        ...
        <div class="form-group">
            ...
            <input type="password" name="password" class="form-control" id="input-password" style="color:#555;background-color:#fff; font-size:14px; margin:0" placeholder="Mot de passe">
        </div>
        <div class="form-group">
            <label for="input-password2">Mot de passe (confirmation)</label>
            <input type="password" name="password2" class="form-control" id="input-password2" style="color:#555;background-color:#fff; font-size:14px; margin:0" placeholder="Mot de passe">
        </div>
        
        <p class="text-center"><button type="submit" class="btn btn-primary">Enregistrer un nouveau client</button> <a href="mon_compte.php" class="btn btn-danger">Retour</a></p>
    </form>
</div>
